...
var VideoEmbed = function VideoEmbed(p) {
  if (_typeof(p) !== 'object') {
    p = {
      src: p
    };
  ...
  var _p = p,
      ...
      height = _p.height,
      _p$class = _p["class"],
      cl = _p$class === void 0 ? 'VideoEmbed' : _p$class,
      style = _p.style,
      host = _p.host,
      props = _objectWithoutProperties(_p, ["src", "width", "height", "class", "style", "host"]);
  ...
  if (!cl.includes('VideoEmbed')) {
    cl = "VideoEmbed ".concat(cl);
  }

  if (src === +src) {
    src = "https://player.vimeo.com/video/".concat(src);
  } else if (!src.startsWith('http')) {
    src = "https://www.youtube-nocookie.com/embed/".concat(src);
  }
  ...
  return div(wrapperProps, iframe(_objectSpread({
    frameborder: 0,
...
var pages = {
  '/video-embed/': function videoEmbed(state) {
    return [h1('@magic-modules/video-embed'), p(['this is the ', Link({
      to: 'https://github.com/magic-modules'
    }, '@magic-modules'), ' VideoEmbed component. It renders videos from any http url that allows iframe embedding.']), GitBadges('magic-modules/video-embed'), h2({
      id: 'installation'
    }, 'installation:'), Pre('npm install @magic-modules/video-embed'), h2({
      id: 'usage'
    }, 'usage'), h3({
      id: 'usage-youtube'
    }, 'youtube'), p('in a page/component, just pass a video id to the VideoEmbed module.'), Pre("VideoEmbed('ecIWPzGEbFc')"), p('renders'), VideoEmbed('ecIWPzGEbFc'), h3({
      id: 'usage-vimeo'
    }, 'vimeo'), p('if the video id is a number, the video is assumed to be hosted on vimeo'), Pre("\nVideoEmbed(153090637)\nVideoEmbed('153090637')\n"), p('renders'), VideoEmbed(153090637), h4({
      id: 'usage-others'
    }, 'other video hosts'), p('videos from arbitrary hosts can be included as well, as long as those hosts allow iframe embeds'), Pre("VideoEmbed('https://videopress.com/embed/kUJmAcSf')"), p('renders'), VideoEmbed('https://videopress.com/embed/kUJmAcSf'), h2({
      id: 'props'
    }, 'props'), p('you can pass a width and height, a style object and any other props to the video.'), Pre("VideoEmbed({ src: '8QYWGCEm2Dc', width: '50%', height: 200, style: { margin: '1em 0 0' } })"), p('renders'), VideoEmbed({
      src: '8QYWGCEm2Dc',
      width: '50%'
    }), h3({
      id: 'fixed-dimensions'
    }, 'fixed video dimensions'), p('to set a fixed video size, simply add both a height and a width value (px, em, rem, vh, pt).'), Pre("VideoEmbed({ src: 'SMhwddNQSWQ', width: '300px', height: '900px' })"), VideoEmbed({
      src: 'SMhwddNQSWQ',
      width: '300px',
      height: '900px'
    }), h2({
  ...
  '/video-embed/404/': function videoEmbed404() {
